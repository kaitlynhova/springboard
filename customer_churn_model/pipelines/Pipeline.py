...
def get_pipeline(
    ...
    input_data = ParameterString(
        name="InputData",
        default_value="s3://{}/data/storedata_total.csv".format(
            default_bucket),
    )
    ...
    step_process = ProcessingStep(
        ...
        outputs=[
            ProcessingOutput(output_name="train", source="/opt/ml/processing/train",
                             destination=f"s3://springboard-aws-ml-hova/output/train"),
            ProcessingOutput(output_name="validation", source="/opt/ml/processing/validation",
                             destination=f"s3://springboard-aws-ml-hova/output/validation"),
            ProcessingOutput(output_name="test", source="/opt/ml/processing/test",
                             destination=f"s3://springboard-aws-ml-hova/output/test")
        ],
        code=f"s3://springboard-aws-ml-hova/input/code/preprocess.py",
    )

    # training step for generating model artifacts
    model_path = f"s3://springboard-aws-ml-hova/output"
    ...
    step_tuning = TuningStep(
        ...
        tuner=HyperparameterTuner(xgb_train, objective_metric_name,
                                  hyperparameter_ranges, max_jobs=2, max_parallel_jobs=2),
        inputs={
            "train": TrainingInput(
    ...
    step_eval = ProcessingStep(
        ...
        inputs=[
            ProcessingInput(
                source=step_tuning.get_top_model_s3_uri(
                    top_k=0, s3_bucket=default_bucket, prefix="output"),
        ...
        outputs=[
            ProcessingOutput(output_name="evaluation", source="/opt/ml/processing/evaluation",
                             destination=f"s3://springboard-aws-ml-hova/output/evaluation"),
        ],
        code=f"s3://springboard-aws-ml-hova/input/code/evaluate.py",
    ...
    transformer = Transformer(
        ...
        instance_count=1,
        output_path=f"s3://springboard-aws-ml-hova/ChurnTransform"
    )
    ...
    bias_report_output_path = f"s3://springboard-aws-ml-hova/clarify-output/bias"
    clarify_instance_type = 'ml.c5.xlarge'
    analysis_config_path = f"s3://springboard-aws-ml-hova/clarify-output/bias/analysis_config.json"
    ...
    step_config_file = ProcessingStep(
        ...
        code=f"s3://springboard-aws-ml-hova/input/code/generate_config.py",
    ...
    data_config = sagemaker.clarify.DataConfig(
        s3_data_input_path=f's3://springboard-aws-ml-hova/output/train/train.csv',
        s3_output_path=bias_report_output_path,
        label=0,
        headers=['target', 'esent', 'eopenrate', 'eclickrate', 'avgorder', 'ordfreq', 'paperless', 'refill', 'doorstep', 'first_last_days_diff', 'created_first_days_diff',
                 'favday_Friday', 'favday_Monday', 'favday_Saturday', 'favday_Sunday', 'favday_Thursday', 'favday_Tuesday', 'favday_Wednesday', 'city_BLR', 'city_BOM', 'city_DEL', 'city_MAA'],
    ...
    step_cond = ConditionStep(
        ...
        if_steps=[step_register, step_create_model,
                  step_config_file, step_transform, step_clarify],
        else_steps=[],
    )
    # pipeline instance
    pipeline = Pipeline(
